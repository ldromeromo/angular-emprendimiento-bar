...
@Component({
  selector: 'app-scale-stateful',
  templateUrl: './scale-stateful.component.html',
  styleUrls: ['./scale-stateful.component.scss'],
  providers: [MessageService]
})
export class ScalestatefulComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.registroForm = this.fb.group({
      ...
      stateful: [null, Validators.required]
    });
  ...
  validarUsuario() {
    ...
    this.http.get<any[]>(url).subscribe((data: any[]) => {
      ...
      const hasstatefulZero = usuarioData.some(item => item.admin === 1);

      // Si tiene admin 0, pasar "all", de lo contrario, pasar el usuario normal
      const usuario = hasstatefulZero ? "all" : this.nh_user;
  ...
  cargarData(usuario) {
    ...
    this.http.get(url_consulta).subscribe((data: any) => {
      ...
      this.allData = data.map(item => ({
        ...
        stateful_id: Number(item.stateful_id)
      }));
  ...
  onClusterChange(clusterId: number): void {
    ...
    this.registroForm.get('stateful').setValue(null);
    this.stateful = [];
  }
  ...
  onNamespaceChange(namespaceId: number): void {
    ...
    this.allData.forEach(item => {
      ...
      if (item.namespace_id === namespaceIdNumber) {
        if (!statefulsMap.has(item.stateful_id)) {
          statefulsMap.set(item.stateful_id, {
            label: item.stateful_name,
            value: item.stateful_id
          });
        }
      }
    });
    this.stateful = Array.from(statefulsMap.values());
  ...
  escalado(replica: number): void {
    ...
    const selectedstateful = this.allData.find(item => item.stateful_id === Number(formData.stateful));

    const data = {
      "cluster": selectedCluster ? selectedCluster.label : null,
      "stateful": selectedstateful ? selectedstateful.stateful_name : null,
      "namespace": selectedNamespace ? selectedNamespace.namespace_name : null,
      "stateful_id": formData.stateful,
    ...
    this.http.post(url, data, { observe: 'response' }).subscribe(
      (response) => {
        ...
        // Verificar si la respuesta contiene "stateful.apps/portalcloud scaled"
